...
class Userlogin extends User {
	...
	Userlogin() throws SQLException {
		...
		if (flag == 0) {
			...
			PreparedStatement userPreparedStatement = connection
					.prepareStatement("insert into userlogindata_tbl(UserMailId,UserName) values(?,?)");
	...
	private void getExam(String useMailId) throws SQLException {
		...
		PreparedStatement MarkUpdatePreparedStatement = connection
				.prepareStatement("update userlogindata_tbl set UserMark = ? WHERE UserMailId= ?");
